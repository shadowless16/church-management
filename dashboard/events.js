    ...
    setTimeout(() => {
        ...
        document.querySelectorAll('.action-btn.edit').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const event = app.data.events.events.find(ev => (ev.id + '') === eventId);
                if (event) {
                    // Ensure form has action="#" to prevent navigation
                    const editEventForm = document.getElementById('edit-event-form');
                    if (editEventForm) {
                        editEventForm.setAttribute('action', '#');
                        ...
                        newEditForm.addEventListener('submit', (e) => {
                            e.preventDefault();
                            app.handleEditEvent(e);
                        });
                        // Fill the new form with event data (including description)
        ...
        // Edit event form (remove duplicate listeners and ensure only one handler)
        const editEventForm = document.getElementById('edit-event-form');
        if (editEventForm) {
            // Remove any previous submit listeners
            const newEditForm = editEventForm.cloneNode(true);
        ...
        // Ensure delete button is not inside a form or is type='button' and only one click handler
            ...
            newBtn.onclick = async function() {
                ...
                if (eventId) {
                    ...
                    try {
                        ...
                        app.closeModal('delete-event-modal');
                        await app.loadData();
                        app.renderPage('events');
                        showToast('Event deleted!', 'success');
                    } catch (err) {
                        // Suppress 404 Not Found errors (treat as success)
