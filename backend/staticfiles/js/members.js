...
export function renderMembers(app, content, headerActions, page = 1, pageSize = 15, filterStatus = 'All', searchTerm = '') {
    ...
    let members = (app.data.members.members || []);
    const stats = app.data.members.stats || {};

    // Filter by status
    if (filterStatus && filterStatus !== 'All') {
        members = members.filter(m => (m.status ?? m.Status ?? '').toLowerCase() === filterStatus.toLowerCase());
    }
    // Search by name, email, phone
    if (searchTerm && searchTerm.trim() !== '') {
        const term = searchTerm.trim().toLowerCase();
        members = members.filter(m =>
            (m.first_name ?? m.firstName ?? '').toLowerCase().includes(term) ||
            (m.last_name ?? m.lastName ?? '').toLowerCase().includes(term) ||
            ...
            <div class="card-header">
                <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                    ...
                    <div style="display: flex; gap: 8px;">
                        <select id="status-filter" style="padding: 6px 12px; border: 1px solid #d1d5db; border-radius: 6px;">
                            ...
                            <option${filterStatus === 'New' ? ' selected' : ''}>New</option>
                        </select>
                ...
                <div style="margin-top: 16px;">
                    <div class="search-container">
                        <input type="text" class="search-input" id="member-search" placeholder="Search members..." value="${searchTerm.replace(/"/g, '&quot;')}">
        ...
        controls.querySelectorAll('.pagination-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (newPage !== currentPage) {
                    renderMembers(app, content, headerActions, newPage, pageSize, filterStatus, searchTerm);
                }
            });
        });
    }
    ...
    if (statusFilter) {
        statusFilter.addEventListener('change', (e) => {
            renderMembers(app, content, headerActions, 1, pageSize, statusFilter.value, searchTerm);
        });
    }

    // Search event listener (preserve focus and cursor position)
    const searchInput = content.querySelector('#member-search');
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            // Store value and cursor position
            const value = searchInput.value;
            const selectionStart = searchInput.selectionStart;
            const selectionEnd = searchInput.selectionEnd;
            // Rerender and restore after
            renderMembers(app, content, headerActions, 1, pageSize, filterStatus, value);
            setTimeout(() => {
                const newInput = content.querySelector('#member-search');
                if (newInput) {
                    newInput.focus();
                    // Restore cursor to end if possible, else fallback
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            // Export currently filtered members (not just paged)
            const csvRows = [];
            const headers = ['First Name','Last Name','Email','Phone','Date of Birth','Gender','Status','Join Date','Ministry','Notes','Profession','Marital Status'];
            ...
            members.forEach(m => {
                const row = [
                    ...
                    Array.isArray(m.ministry) ? m.ministry.join('; ') : '',
                    m.notes ?? '',
                    m.profession ?? '',
                    m.marital_status ?? ''
